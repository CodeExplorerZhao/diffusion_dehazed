...
#     """
#     Channel attention for LL branch.
#     Used to reweight low-frequency channels according to global statistics.
#     """
#     def __init__(self, channels, reduction=None):
#         super().__init__()
#         reduction = reduction or max(16, channels // 2)
...
#     """
#     Grouped spatial attention for HF branch.
...
#         self.conv = nn.Conv2d(2, 3, kernel_size, padding=padding, bias=False)
...
#         return self.sigmoid(self.conv(attn))              # [B,3,H,W]

class GroupedSpatialAttention(nn.Module):
    """
    修正后的独立分频空间注意力：
    1. 拒绝过早混合：分别为 LH, HL, HH 计算各自的空间掩码。
    2. 保持标准算子：使用各向同性的卷积核，仅改变掩码提取逻辑。
    """
    def __init__(self, kernel_size=7):
        super().__init__()
        padding = kernel_size // 2
        # 为每个方向独立定义掩码生成卷积（输入 2 通道：mean+max，输出 1 通道权重）
        self.attn_lh = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
        self.attn_hl = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
        self.attn_hh = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, lh, hl, hh):
        def get_subband_mask(x, conv_op):
            # 仅在子带内部计算均值和最大值，确保权重生成的依据是纯净的方向特征
            avg_out = torch.mean(x, dim=1, keepdim=True)
            max_out, _ = torch.max(x, dim=1, keepdim=True)
            attn_input = torch.cat([avg_out, max_out], dim=1)
            return self.sigmoid(conv_op(attn_input))

        return get_subband_mask(lh, self.attn_lh), \
               get_subband_mask(hl, self.attn_hl), \
               get_subband_mask(hh, self.attn_hh)
...
class LLBlock(nn.Module):
    ...
    def __init__(self, n_feat):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(n_feat, n_feat, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(n_feat, n_feat, 3, padding=1), #。, dilation=2
            nn.ReLU(inplace=True),
            nn.Conv2d(n_feat, n_feat, 3, padding=1)
        )
...
class HFBlock(nn.Module):
    ...
    def forward(self, lh, hl, hh, scale):
        ...
        if self.use_group_gate:
            g_lh, g_hl, g_hh = self.gate(lh, hl, hh)
